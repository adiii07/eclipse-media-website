...
export default function Home() {
  const [activeSection, setActiveSection] = useState<"artists" | "brands">(
    "artists",
  );
    ...
    <>
      ...
      <div className="min-h-screen bg-black">
        ...
        <section className="relative h-screen w-full flex flex-col items-center justify-center px-6">
          ...
          >
            <motion.h1
              className="font-extralight text-[#a5cdcf] text-left leading-[1.05]"
              style={{
            ...
            >
              <span className="block text-6xl md:text-8xl lg:text-9xl tracking-[0.08em]">
                ECLIPSE
              </span>
              <span className="block text-5xl md:text-7xl lg:text-8xl tracking-[0.08em]">
                MEDIA
              </span>
            </motion.h1>

            <motion.p
              className="text-xl md:text-2xl font-light tracking-widest text-white/90"
              initial={{
                opacity: 0,
                y: 20,
              }}
          ...
          <motion.div
            className="absolute bottom-24 md:bottom-28 flex flex-col items-center justify-center gap-6"
            initial={{
              opacity: 0,
              y: 20,
            }}
          ...
          >
            <div className="relative flex items-center bg-white/10 backdrop-blur-sm rounded-full p-1 border border-white/20">
              ...
              <button
                ...
                className={`relative z-10 px-6 md:px-8 py-2.5 text-xs md:text-sm font-light tracking-widest uppercase transition-colors duration-300 ${activeSection === "artists" ? "text-black" : "text-white/70 hover:text-white"}`}
              >
                For Artists
              </button>
              <button
                onClick={() => handleSectionChange("brands")}
                className={`relative z-10 px-6 md:px-8 py-2.5 text-xs md:text-sm font-light tracking-widest uppercase transition-colors duration-300 ${activeSection === "brands" ? "text-black" : "text-white/70 hover:text-white"}`}
              >
                For Brands
              </button>
            </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeSection === "artists"
                  ? "Stories of artists, captured."
                  : "Where brands become stories."}
              </motion.p>
            </AnimatePresence>
          </motion.div>
        </section>
